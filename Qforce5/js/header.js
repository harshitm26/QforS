...
function findplace(arg)
{
	if (arg == 1)
		var addr = $('#user_exact_address').val();
	else
		var addr = $('#ngo_exact_address').val();
	

	
	var url = "http://universimmedia.pagesperso-orange.fr/geo/loc.htm?address=" + addr + "#";


	window.open(url,"_blank","toolbar=yes, location=yes, directories=no, status=no, menubar=yes, scrollbars=yes, resizable=no, copyhistory=yes, width=700, height=900");
...
function suggest_places()
{
		window.open("suggest_places.php","_blank","width=700, height=700");
}
...
function ngologin(){
	...
	//~ var objectquota=document.getElementById('quota');
	//~ var quota= objectquota.options[objectquota.selectedIndex].value;
	if(document.getElementById('login_uniqueid') != null)
		var params= "login_uniqueid="+document.getElementById('login_uniqueid').value + "&login_password=" + document.getElementById('login_password').value;
	else
		var params = "";
	...
	temp.onreadystatechange=function(){
		if(temp.readyState==4 && temp.status==200){
			...
			document.getElementById('sidediv1').style.display = 'none';
			document.getElementById('ngo_login_div').style.display = 'none';
			document.getElementById('ngo_teach_div').style.display = 'none';
...
function ngo_teaching()
{
	//alert("NGO Teaching");
	if (document.getElementById('ngo_exact_place') != null)
	{
		var params ='ngo_exact_place='+document.getElementById('ngo_exact_place').value + '&ngo_date='+ document.getElementById('ngo_date').value+ '&ngo_num_people='+ document.getElementById('ngo_num_people').value+ '&ngo_start_time='+ document.getElementById('ngo_start_time').value+ '&ngo_end_time='+ document.getElementById('ngo_end_time').value;

		temp.open("POST", "ngo_teaching.php", true);
		temp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		temp.setRequestHeader("Content-length", params.length);
		temp.setRequestHeader("Connection", "close");
		temp.onreadystatechange=function(){
			if(temp.readyState==4 && temp.status==200){
				document.getElementById('ngo_login_error').innerHTML = '';
				//~ document.getElementById('ngo_login_div').style.display = 'none';
				
				document.getElementById('ngo_teach_div').style.display = 'inline';
				document.getElementById('ngo_teach_div').innerHTML=temp.responseText;
	...
	else
	{
		...
		temp.onreadystatechange=function(){
			if(temp.readyState==4 && temp.status==200){
				...
				$('#ngo_start_time').ptTimeSelect();
				$('#ngo_end_time').ptTimeSelect();
...
function user_display_teach_info()
{
		//alert("DTI");
		var params ='';

		temp.open("POST", "display_teach_info.php", true);
		temp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		temp.setRequestHeader("Content-length", params.length);
		temp.setRequestHeader("Connection", "close");
		temp.onreadystatechange=function(){
			if(temp.readyState==4 && temp.status==200){		
					...
					document.getElementById('user_login_error').innerHTML=temp.responseText + "<input class='button_example' style='width:200;height:40' type='button' onclick='userlogin();' value='Back' />";
...
function ngo_display_teach_info()
{
		//alert("DTI");
		var params ='';
		...
		temp.onreadystatechange=function(){
			if(temp.readyState==4 && temp.status==200){		
				...
				document.getElementById('ngo_login_div').style.display = 'none';
				document.getElementById('ngo_teach_div').style.display = 'inline';
				document.getElementById('ngo_teach_div').innerHTML=temp.responseText+ "<input class='button_example' style='width:200;height:40' type='button' onclick='ngologin();' value='Back' />";;
